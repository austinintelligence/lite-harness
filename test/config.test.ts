...
describe("versioned application configuration", () => {
  it("parses bounded core Manager and Gateway configuration", () => {
    ...
    const gateway = loadGatewayConfiguration({ ...tokens, LITE_HARNESS_PORT: "4321" }, "C:/fixture", "win32");
    expect(gateway).toMatchObject({ schemaVersion: 1, host: "127.0.0.1", port: 4321 });
  });
