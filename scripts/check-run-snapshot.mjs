...
try {
  ...
  if (!report) throw new Error("RunSnapshot suite did not produce JSON reporter output");
  ...
  if (evidenceOutput) {
    writeVitestEvidence({
      ...
      claims: {
        boundaries: {
          frozenBeforeFirstModelTurn: true,
          immutablePerAttempt: true,
          exactToolsSkillsPluginsAndPolicies: true,
          credentialIdsOnly: true,
        },
      },
    });
  }
  if (!passed) throw new Error("RunSnapshot suite did not produce a zero-skip pass");
