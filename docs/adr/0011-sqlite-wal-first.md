# ADR 0011: SQLite WAL as the first database

- Status: accepted
- Date: 2026-07-14
- Requirement: D11
- Contract: `LITE_HARNESS_ARCHITECTURE_PLAN.md`, 5. Architecture decisions to freeze, lines 192-192
...
> SQLite in WAL mode is the first database.
...
- Rejected: requiring an external database for the local-first alpha. It weakens the selected local-first boundary or creates a second architecture to support.
