...
const temporary = mkdtempSync(resolve(tmpdir(), "lite-status-truth-report-"));
const reportPath = resolve(temporary, "vitest.json");
try {
  const vitest = spawnSync(process.execPath, [
    resolve(root, "node_modules", "vitest", "vitest.mjs"),
    "run", "test/release-truth.test.ts", "--reporter=json", `--outputFile=${reportPath}`,
  ], { cwd: root, stdio: "inherit" });
  const report = existsSync(reportPath) ? JSON.parse(readFileSync(reportPath, "utf8")) : undefined;
  if (!report) throw new Error("Status truth suite did not produce JSON reporter output");
  ...
  if (evidenceOutput) {
    writeVitestEvidence({
      ...
      suite: "m11-status-truth",
      command: "pnpm check:status-truth",
      report: evidenceReport,
      regressionIds: ["BD-063-REGRESSION"],
      claims: {
        documentation: {
          architectureAndBlockerDocsDeclareUnverifiedAlpha: true,
          staleEvidenceAndUnprovenClosuresRemainBlocked: true,
        },
      },
    });
  }
  if (!passed) throw new Error("Status truth suite did not produce a zero-skip pass");
  process.stdout.write(`Status truth checks passed (${report.numPassedTests}/${report.numTotalTests}, zero skips).\n`);
} finally {
