...
try {
  ...
  if (report.success !== true || report.numFailedTests !== 0 || report.numPendingTests !== 0 || report.numTodoTests !== 0) {
    throw new Error("SSE and IPC backpressure suite did not produce a zero-skip pass");
  }
...
function writeEvidence(output, report) {
  ...
  const document = {
    schemaVersion: 1,
    evidenceId: `m2-backpressure-${commit.slice(0, 12)}-${platform()}-${arch()}`,
    commit,
