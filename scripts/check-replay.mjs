...
try {
  ...
  if (evidenceOutput) {
    writeVitestEvidence({
      ...
      suite: "m2-reconnect-replay",
      command: "pnpm test:replay",
      report: evidenceReport,
      requirementIds: ["A03"],
      claims: {
        boundaries: {
          realGatewaySocketTerminated: true,
          gatewayRestartedOnSameAddress: true,
          managerStatePreservedAcrossRestart: true,
          cursorResumed: true,
          orderedGapFreeDelivery: true,
          duplicateSuppression: true,
          noRunReexecution: true,
        },
      },
    });
  }
  if (!passed) throw new Error("Reconnect/replay suite did not produce a zero-skip pass");
  process.stdout.write(`Reconnect/replay checks passed (${report.numPassedTests}/${report.numTotalTests}, zero skips).\n`);
} finally {
