...
Codex app-server does not expose a native monetary cap, so runs with the
mandatory dollar budget also require the two model USD-per-million-token price
variables documented in `PROVIDERS.md`. Set both to zero only when the selected
subscription route is genuinely zero marginal cost. Missing pricing fails
before the delegated process starts.
...
The per-run budget is also passed as a CLI-native cap, using the lower value
when both are present. A completed run without enforceable cost usage fails
closed instead of being recorded as free.
