...
function render() {
  ...
  const declaredCounts = countBy(requirements, "status");
  ...
  const verified = currentVerified.length === requirements.length && openDefects.length === 0 && unprovenClosures.length === 0;
  const lines = [
    ...
    `- Currently blocked or unverified: ${requirements.length - currentVerified.length}`,
